 ...
 */
final class MyCameraConfigurationManager {
    ...
    MyCameraConfigurationManager(Context context) {
    ...
    void initFromCameraParameters(Camera camera) {
        ...
        // and reverse them:
        if (width < height) {
            _Log.i(TAG, "Display reports portrait orientation; assuming this is incorrect");
            int temp = width;
            width = height;
            height = temp;
        }
        screenResolution = new Point(width, height);
        _Log.i(TAG, "Screen resolution: " + screenResolution);
        cameraResolution = findBestPreviewSizeValue(parameters,
                screenResolution);
        _Log.i(TAG, "Camera resolution: " + cameraResolution);
    }

    void setDesiredCameraParameters(Camera camera, boolean safeMode) {
        Camera.Parameters parameters = camera.getParameters();
        if (parameters == null) {
            _Log.w(TAG, "Device error: no camera parameters are available. Proceeding without configuration.");
            return;
        }
        _Log.i(TAG, "Initial camera parameters: " + parameters.flatten());
        if (safeMode) {
            _Log.w(TAG, "In camera config safe mode -- most settings will not be honored");
        }
    ...
    private Point findBestPreviewSizeValue(Camera.Parameters parameters,
        ...
        if (rawSupportedSizes == null) {
            _Log.w(TAG, "Device returned no supported preview sizes; using default");
        ...
        //_Log.i(TAG, "Supported preview sizes: " + previewSizesString);
        ...
        for (Camera.Size supportedPreviewSize : supportedPreviewSizes) {
            ...
            if (maybeFlippedWidth == screenResolution.x
                ...
                //_Log.i(TAG, "Found preview size exactly matching screen size: " + exactPoint);
                return exactPoint;
            }
        ...
        //_Log.i(TAG, "Found best approximate preview size: " + bestSize);
        return bestSize;
    }
    ...
    private static String findSettableValue(Collection<String> supportedValues,
        ...
        //_Log.i(TAG, "Settable value: " + result);
        return result;
    }
